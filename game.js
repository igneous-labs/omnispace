...
let SpriteSheetFrameMap = {
  ...
  char_seulgi: {
    {
      standing: [ [ 1, 110 ], [ 112, 221 ] ],
      walking: [ [ 223, 335 ], [ 337, 446 ], [ 448, 560 ], [ 562, 671 ] ]
    }
  }
};
...
function parseWorldStateDataEntry(arrayBuffer) {
  ...
  return [
    dataView.getUint16(0, true),
    {
      ...
      direction:
        dataView.getUint8(14, true) === DIRECTION_LEFT ? "left" : "right",
      status:
        dataView.getUint8(15, true) === STATUS_IDLE ? "standing" : "walking",
    },
  ];
}
      ...
      event.data.arrayBuffer().then((arrayBuffer) => {
        ...
        switch (messageType) {
          case ACKNOWLEDGE_MESSAGE_TYPE:
            ...
            console.log(
              `[NetworkHandler::on_message] sending player chat user id: ${MATRIX_USER_ID}`,
            );
          ...
          case WORLD_STATE_MESSAGE_TYPE:
            ...
            const [_, instanceChatUserIdsBytes] = slicePackedByteArray(
              arrayBuffer,
